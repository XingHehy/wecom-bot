...
def register_routes(app: FastAPI, get_agent_runtime: Callable[[str], AgentRuntime]):
    ...
    async def root(agent_id: str):
        if agent_id not in AGENT_CONFIGS:
            raise HTTPException(status_code=404, detail="无效的agent_id")
        agent_runtime = get_agent_runtime(agent_id)
        return {
            "status": "running",
            "service": "企业微信智能对话系统",
            "description": "基于 LangChain 1.x agent runtime 的企业微信消息处理服务",
            "author": "xinghehy",
            "agent_id": agent_id,
        ...
        try:
            ...
            async def process_and_send_replies_task(message_content):
                try:
                    ...
                    logger.debug(f"调用 LangChain agent runtime 处理消息，runtime: {agent_runtime.status()}")
                    result = await agent_runtime.ainvoke(message_content, from_user, context)
                    replies = result.replies
                    logger.debug(f"agent runtime 返回结果: selected={result.selected_agent}, replies={replies}")
                    if not replies:
